...
analysis_type=finetune0_template #finetune0_template or finetune1_template
...
cp template_dir/${analysis_type}/A01.bpp${i}.ctl $run_dir/A01.bpp${i}_run${j}.ctl

cp $phylip_data $data_dir

cp ${imap_data} $data_dir

sed -i "s,template_input_path,${data_dir},g" $run_dir/A01.bpp${i}_run${j}.ctl
sed -i "s/seq_file_name/${phylip_data}/g" $run_dir/A01.bpp${i}_run${j}.ctl
sed -i "s/Imap_file_name/${imap_data}/g" $run_dir/A01.bpp${i}_run${j}.ctl

sed -i "s,template_output_path,${run_dir},g" $run_dir/A01.bpp${i}_run${j}.ctl
sed -i "s/n_partitions/${number_loci}/g" $run_dir/A01.bpp${i}_run${j}.ctl
...
control_file=$run_dir/A01.bpp${i}_run${j}.ctl

cd $run_dir

sbatch --job-name=comb${i}_run${j}_A01 ./run_bpp.sh $control_file
